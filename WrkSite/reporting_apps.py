...
class database_connections:
    ...
    def df_sql_shared(self, start_date, end_date):
        ...
        shared_sql = """select *
          	from IP.tickets_resolved tr
          		inner join IP.im im on im.ticket_id=tr.ticket_id
          		left join IP.mon mon on (mon.ticket_id = tr.ticket_id and mon.state != "UNKNOWN")
          		left join IP.rfcm cm on cm.ticket_id=tr.ticket_id
          		left join IP.proj pj on pj.ticket_id=tr.ticket_id
          		left join IP.ipsec_one ipsec on ipsec.ticket_id=tr.ticket_id


          	where
          		tr.create_date BETWEEN DATE('{date}') AND DATE('{end_date}') AND
          		cr.realname != "IP IPcalen" AND
          		 not lower(iq.name) REGEXP '.*project.*'
          ) all_tickets
          where 'all_tickets.`department`  not in 'Marketing',
...
class create_megalog:
    def __init__(self, team, IN_FOLDER, OUT_FOLDER):
        ...
        if re.match('.*Apps.*', self.team):
            self.rawMetrics_file_name = 'Research_Tlog_Megalog'
            self.rawMetrics_team_name = 'Researchs Management|Researchs Management'
        if re.match('.*Ent.*', self.team):
            self.rawMetrics_file_name = 'Sales_Tlog_Megalog'
            self.rawMetrics_team_name = 'Sales Researchs|Sales Researchs'
...
class pivot:
    ...
    def pvt_eng_resolved(self, writer, sheet_name, dataframe, position):
        ...
        engineer_roster = ['Names', 'Names']
        pos = position
        result = result[result.Client.str.contains('M')]
...
class create_df:
    ...
    def data_by_client_instance(self, dataframe):
        ...
        M_df = dataframe[dataframe['Client'].str.contains('M')]
        V_df = dataframe[dataframe['Client'].str.contains('V')]
        I_df = dataframe[dataframe['Client'].str.contains('I')]
        return shared_df, M_df, V_df, I_df

    def score(self, dataframe):
        shared_df, M_df, V_df, I_df = self.data_by_client_instance(dataframe)
        shared_df
...
class reporting(create_megalog):
    def __init__(self, team):
        ...
        self.RFC_REPORT_FILE = 'M8'
        self.attendance = 'Shared'
    ...
    def list_of_reports(self, MASTER_WRITER, RFC_REPORT_FILE):
        ...
        shared_df, MisCompanyrd_df, V_Company_df, I_Company_df = self.cdf.data_by_client_instance(master_dataframe)
        master_dataframe = pd.concat([shared_df, MisCompanyrd_df, V_Company_df, I_Company_df], axis=0)

        # print(master_dataframe.head())
        # # Working Reports
        self.Volume_Charts_Pie(MASTER_WRITER, master_dataframe)
        self.Attendance_Reports(MASTER_WRITER)
        self.Column_Charts(MASTER_WRITER, MisCompanyrd_df)
    ...
    def Volume_Charts_Pie(self, MASTER_WRITER, master_dataframe):
        ...
        MisCompanyrd_df = master_dataframe[master_dataframe['Client'].str.contains('M')]
        NotMisCompanyrd_df = master_dataframe[~master_dataframe['Client'].str.contains('M|A|P')]
        ...
        self.title1 = 'Ticket Volume by Client\n(Exluding MisCompanyrd)\n Total:'
        total, dataframe = self.pvt.pvt_pie1_vol_by_client_other(NotMisCompanyrd_df, writer, VOLUME_SHEET, position)
        ...
        total, dataframe = self.pvt.pvt_pie2_vol_by_shift_client_other(NotMisCompanyrd_df, writer, VOLUME_SHEET, position)
        ...
        total, dataframe = self.pvt.pvt_pie3_vol_by_shift_client_other(NotMisCompanyrd_df, writer, VOLUME_SHEET, position)
        ...
        total, dataframe = self.pvt.pvt_pie4_vol_by_shift_client_other(NotMisCompanyrd_df, writer, VOLUME_SHEET, position)
        ...
        # P1
        position = pvt_position_tuple.pop()
        total, dataframe = self.pvt.pvt_pie1_miss_sla_client_other(NotMisCompanyrd_df, writer, MISS_SLA_SHEET, position)
        self.grh.pie_vol_by_client_other(dataframe, writer, MISS_SLA_SHEET, position, total, title1, percentage=True)
        # P2
        position = pvt_position_tuple.pop()
        total, dataframe = self.pvt.pvt_pie2_miss_sla_client_other(NotMisCompanyrd_df, writer, MISS_SLA_SHEET, position)
        self.grh.pie_vol_by_client_other(dataframe, writer, MISS_SLA_SHEET, position, total, title2, percentage=True)
        # P3
        position = pvt_position_tuple.pop()
        total, dataframe = self.pvt.pvt_pie3_miss_sla_client_other(NotMisCompanyrd_df, writer, MISS_SLA_SHEET, position)
        ...
        # P1
        position = pvt_position_tuple.pop()
        total, dataframe = self.pvt.pvt_pie1_vol_by_client_other(MisCompanyrd_df, writer, VOLUME_SHEET_MC, position)
        self.grh.pie_vol_by_client_other(dataframe, writer, VOLUME_SHEET_MC, position, total, title1, percentage=True)
        # P2
        position = pvt_position_tuple.pop()
        total, dataframe = self.pvt.pvt_pie2_vol_by_shift_client_other(MisCompanyrd_df, writer, VOLUME_SHEET_MC, position)
        self.grh.pie_vol_by_client_other(dataframe, writer, VOLUME_SHEET_MC, position, total, title2, percentage=True)
        # P3
        position = pvt_position_tuple.pop()
        total, dataframe = self.pvt.pvt_pie3_vol_by_shift_client_other(MisCompanyrd_df, writer, VOLUME_SHEET_MC, position)
        self.grh.pie_vol_by_client_other(dataframe, writer, VOLUME_SHEET_MC, position, total, title3, percentage=True)
        # P4
        position = pvt_position_tuple.pop()
        total, dataframe = self.pvt.pvt_pie5_vol_by_shift_client_other(MisCompanyrd_df, writer, VOLUME_SHEET_MC, position)
        ...
        # P1
        position = pvt_position_tuple.pop()
        total, dataframe = self.pvt.pvt_pie1_miss_sla_client_other(MisCompanyrd_df, writer, MISS_SLA_SHEET_MC, position)
        self.grh.pie_vol_by_client_other(dataframe, writer, MISS_SLA_SHEET_MC, position, total, title1, percentage=True)
        # P2
        position = pvt_position_tuple.pop()
        total, dataframe = self.pvt.pvt_pie2_miss_sla_client_other(MisCompanyrd_df, writer, MISS_SLA_SHEET_MC, position)
        self.grh.pie_vol_by_client_other(dataframe, writer, MISS_SLA_SHEET_MC, position, total, title2, percentage=True)
        # P3
        position = pvt_position_tuple.pop()
        total, dataframe = self.pvt.pvt_pie3_miss_sla_client_other(MisCompanyrd_df, writer, MISS_SLA_SHEET_MC, position)
        self.grh.pie_vol_by_client_other(dataframe, writer, MISS_SLA_SHEET_MC, position, total, title3, percentage=True)
        # P4
        position = pvt_position_tuple.pop()
        total, dataframe = self.pvt.pvt_pie4_miss_sla_by_client_ex_mc(MisCompanyrd_df, writer, MISS_SLA_SHEET_MC,
                                                                      position)
    ...
    def Column_Charts(self, MASTER_WRITER, master_dataframe):
        ...
        # Variable
        writer = MASTER_WRITER
        MisCompanyrd_df = master_dataframe
        # MisCompanyrd_df = master_dataframe[master_dataframe['Client'].str.contains('MisCompanyrd')]
        # NotMisCompanyrd_df = master_dataframe[~master_dataframe['Client'].str.contains('MisCompanyrd|AtosUK')]
        sheet_name = 'MisCompanyrd'
        # print(MisCompanyrd_df)
        pvt_position_tuple = [[0, 104], [0, 80], [0, 55], [0, 30], [0, 5]]

        # C1
        position = pvt_position_tuple.pop()
        title = 'SLA Missed by Automata vs Engineer'
        dataframe = self.pvt.pvt_col1_eng_auto_miss_vol(MisCompanyrd_df, writer, sheet_name, position)
        self.grh.cht_eng_auto_miss_vol1(dataframe, writer, sheet_name, position, title)
        # C2
        position = pvt_position_tuple.pop()
        title = 'Volume of Tickets Automata vs Engineer'
        dataframe = self.pvt.pvt_col2_eng_auto_miss_vol(MisCompanyrd_df, writer, sheet_name, position)
        self.grh.cht_eng_auto_miss_vol2(dataframe, writer, sheet_name, position, title)
        # C3
        position = pvt_position_tuple.pop()
        title = 'Volume of Tickets by Shift'
        dataframe = self.pvt.pvt_col3_eng_auto_miss_vol(MisCompanyrd_df, writer, sheet_name, position)
    ...
    def RFC_Charts(self, MASTER_WRITER, RFC_REPORT_FILE):
        ...
        sheet_name = 'MisCompanyrd RFC Distribution'
        pvt_position_tuple = [[0, 104], [0, 80], [0, 55], [0, 30], [0, 5]]
        try:
    ...
    def PVT_Charts(self, MASTER_WRITER, master_dataframe, team):
        ...
        self.pvt.pvt2_line_client_missed_auto_owned(MASTER_WRITER, sheet_name, NotMisCompanyrd_df, position)
        # ------------
    ...
    def apps_chart_reports(self, MASTER_WRITER):
        if re.match('.*Apps.*', self.team):
            ...
            # GET FROM DB
            i_sql = "Select * from ScoreCard WHERE Department='Researchs Management' and Platform!='CARESTREAM';"
        ...
        if re.match('.*Ent.*', self.team):
            ...
            # GET FROM DB
            i_sql = "Select * from ScoreCard WHERE Department='Sales Researchs' and Platform!='CARESTREAM';"
            engine = database_connections('LAB_LOCAL').create_sql_engine()
    ...
    def automata_mc_chart(self, MASTER_WRITER):
        ...
        engine = database_connections('LAB_LOCAL').create_sql_engine()
        #######################################
        sql = 'select * from Weekly_RawMetrics_II where Client=\'MisCompanyrd\''
...
class update_database(database_connections):
    ...
    def update_megalog(self, report_date, team):
        if team == 'Ent':
            regteam = "%Sales Researchs%"
        if team == 'Apps':
            regteam = "%Researchs Management%"
...
if __name__ == "__main__":
    Sales_reporting = reporting('Ent')
    Research_reporting = reporting('Apps')
    Research_reporting.OUT_FOLDER = '/home/amcdowald/OPs_Metrics/out/'
    Research_reporting.IN_FOLDER = '/home/amcdowald/OPs_Metrics/in/'
    Sales_reporting.OUT_FOLDER = '/home/amcdowald/OPs_Metrics/out/'
    Sales_reporting.IN_FOLDER = '/home/amcdowald/OPs_Metrics/in/'
    Research_reporting.RFC_REPORT_FILE = 'MC RFC Report-Jan7-Jan13-2018'
    Research_reporting.attendance = 'Shared Apps Team  Attendance - Jan 7 to Jan 13 - 2018'
    # functions().test()

    # Research_reporting.testFunc()
    # create_megalog('Apps',Research_reporting.IN_FOLDER,Research_reporting.OUT_FOLDER).create_tlog()
    ...
    if update:
        ...
        update_database(Research_reporting.team, Research_reporting.IN_FOLDER,
                        Research_reporting.OUT_FOLDER).update_megalog(report_date, 'Apps')
        update_database(Research_reporting.team, Research_reporting.IN_FOLDER,
                        Research_reporting.OUT_FOLDER).upload_ScoreCard(report_date, 'Apps')
        print("UPLOADING ENT")
        update_database(Sales_reporting.team, Sales_reporting.IN_FOLDER,
                        Sales_reporting.OUT_FOLDER).update_megalog(report_date, 'Ent')
        update_database(Sales_reporting.team, Sales_reporting.IN_FOLDER,
                        Sales_reporting.OUT_FOLDER).upload_ScoreCard(report_date, 'Ent')

    print('STARTING')
    create_megalog('Apps', Research_reporting.IN_FOLDER, Research_reporting.OUT_FOLDER).create_tlog()
    Research_reporting.run_reporting()
    print('STARTING')
    Sales_reporting.OUT_FOLDER = '/home/amcdowald/OPs_Metrics/out/'
    Sales_reporting.IN_FOLDER = '/home/amcdowald/OPs_Metrics/in/'
    create_megalog('Ent', Sales_reporting.IN_FOLDER, Sales_reporting.OUT_FOLDER).create_tlog()
    Sales_reporting.run_reporting()
